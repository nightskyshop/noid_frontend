...
try {
  ...
  # 4x6 in = 400 x 600 (hundredths of an inch). 프린터가 지원하는 "테두리없음(borderless)"
  # 4x6 용지가 있으면 우선 사용(있으면 진짜 여백 없이 출력됨), 없으면 일반 4x6 사용.
  $sel = $null
  foreach ($ps in $doc.PrinterSettings.PaperSizes) {
    if ($ps.PaperName -match "4.*6" -and $ps.PaperName -match "(?i)borderless|frameless|full.?bleed|테두리") { $sel = $ps; break }
  }
  if ($sel -eq $null) {
    foreach ($ps in $doc.PrinterSettings.PaperSizes) {
  ...
  $doc.add_PrintPage({
    param($s, $e)
    # 중요: Margins 를 0 으로 설정했기 때문에 MarginBounds/PageBounds 는 둘 다
    # "전체 용지"(400x600) 크기다. 하지만 프린터는 하드웨어 여백 안쪽(PrintableArea)
    # 에만 인쇄할 수 있으므로, 전체 용지 기준으로 그리면 그 여백만큼 오른쪽/아래가
    # 잘린다. 수동 인쇄가 안 잘리는 것은 인쇄 대화상자가 PrintableArea 기준으로
    # 맞춰 주기 때문 — 코드도 동일하게 PrintableArea 크기에 맞춰 그린다.
    # (OriginAtMargins=false 이면 Graphics 원점(0,0)이 PrintableArea 좌상단이므로
    #  (0,0)~(pa.Width, pa.Height) 안에 그리면 절대 잘리지 않는다)
    ...
    # 하드웨어 여백은 상하/좌우가 비대칭일 수 있어, 인쇄 가능 영역 기준으로
    # 중앙 정렬하면 용지 기준으로는 한쪽으로 치우친다. "용지 중앙" 에 놓으면서도
    # 인쇄 가능 영역을 벗어나지 않도록, 큰 쪽 여백을 양쪽에 적용한 대칭 박스를
    # 안전 영역으로 잡는다.
    $marL = $paX
    $marT = $paY
    ...
    # 용지 기준 중앙 좌표를 Graphics 좌표(원점 = 인쇄 가능 영역 좌상단)로 변환
