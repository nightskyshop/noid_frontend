...
try {
  ...
  $doc.add_PrintPage({
    param($s, $e)
    # 중요: OriginAtMargins=false 이면 Graphics 의 (0,0) 은 "인쇄 가능 영역"(프린터
    # 하드웨어 여백 안쪽)의 좌상단이다. 여기서 PageBounds(여백 포함 "전체 용지" 크기)
    # 를 기준으로 크기를 계산하면, 하드웨어 여백만큼 이미지가 인쇄 가능 영역 밖으로
    # 삐져나가 오른쪽/아래가 잘린다. 반드시 MarginBounds(실제 인쇄 가능한 영역 크기)
    # 를 기준으로 계산해야 잘리지 않는다.
    $b = $e.MarginBounds
...
async function printOnWindows(cfg, filePath) {
  ...
  await fs.promises.writeFile(scriptPath, WINDOWS_PRINT_SCRIPT, "utf8");
